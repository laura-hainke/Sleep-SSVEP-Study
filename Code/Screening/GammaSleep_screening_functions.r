...
score_uMCTQ <- function(data) {
  ...
  data_uMCTQ = data[1,grepl("mumctq",colnames(data))]
...
score_PSQI <- function(data) {
  ...
  data_PSQI = data[1,grepl("psqi",colnames(data))]
  ...
  if (!is.na(data_PSQI$psqi_5other) && data_PSQI$psqi_5other != "") {
...
check_data <- function(data,check) {
  ...
  if (check == 'NA_values') {
    ...
    # If actual NA values list is longer than accepted list, print
    if (length(data_na) > length(ok_na)) {
      cat("\nWARNING: columns with NA values!\n")
      print(data_na[!(data_na %in% ok_na)])
    } else {
