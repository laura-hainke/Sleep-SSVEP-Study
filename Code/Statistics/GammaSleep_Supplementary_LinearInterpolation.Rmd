---
title: "GammaSleep Supplementary Analysis: Linear Interpolation"
...
path_derivatives <<- "C:\\Users\\Mitarbeiter\\Documents\\Gamma_Sleep\\Data\\Derivatives_interpolated\\" 
...
The image below shows an SSVEP particularly affected by this issue, with one downward and one upward peak distorting the underlying sinusiodal waveform. The red dashed lines simbolize the linear interpolation procedure: Essentially, data at the beginning (timepoints [-1;3]) and middle (timepoints [11;14]) of each segment is replaced with a straight line. Averaging across "cleaned" segments then results in a sinusoidal form, without residuals of LED ON/OFF artifacts.  

![linear interpolation explanation](C:/Users/Mitarbeiter/Documents/Gamma_Sleep/Github_Repo/Gamma-Sleep/Code/Statistics/linear_interpolation.png)
This procedure was not defined in the preregistration, since pilot data were not affected by artifacts. In fact, neither was the majority of the datasets. Additionally, the presence or absence of artifacts seemed to be constant within participants, and comparisons between conditions were carried out within-subjects. For those reasons, linear interpolation was not applied to main analyses. Nevertheless, since some SSVEPs showed these characteristic peaks, here we report results with this processing step applied. In short, **the results of confirmatory analyses do not change**.  
